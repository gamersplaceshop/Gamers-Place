<?php
/**
 * Template for displaying the "3 Level Dark" header
 *
...
<header id="masthead" role="banner" class="site-header box-shadow-sm site-header-v3-dark">
	<?php if ( apply_filters('cartzilla_enable_topbar' , true )): ?>
		<div class="<?php cartzilla_topbar_class( 'light' === cartzilla_topbar_skin() ? 'topbar-light bg-secondary' : 'topbar-dark bg-darker' ); ?>">
				...
				<div class="ml-3 text-nowrap">
					...
					<?php if ( apply_filters( 'cartzilla_enable_topbar_language_currency_dropdown',  get_theme_mod( 'enable_topbar_language_currency', 'no' ) ) === 'yes' && has_filter( 'cartzilla_dropdown_tools_toggle' ) ) : ?>
						<div class="topbar-text dropdown disable-autohide ml-4">
							...
							</a>							<?php if( has_action( 'cartzilla_dropdown_tools' ) ) : ?>
	...
	<?php endif;?>
	<div class="bg-dark <?php echo cartzilla_header_is_sticky() ? 'navbar-sticky' : ''; ?>">
		<div class="<?php cartzilla_navbar_class( 'navbar-dark' ); ?>">
					...
					<?php if ( cartzilla_navbar_is_account() ) : ?>
						<?php if ( is_user_logged_in() ) : ?>
							<div class="navbar-tool dropdown ml-2">
								<a class="navbar-tool-icon-box border border-light dropdown-toggle" href="<?php echo get_permalink( wc_get_page_id( 'myaccount' ) ); ?>">
		...
		<div class="<?php cartzilla_navbar_class( 'navbar-dark mt-n2 pt-0 pb-2' . ( cartzilla_header_is_sticky() ? ' navbar-stuck-menu' : '' ) ); ?>">
			<div class="<?php echo cartzilla_header_is_fw() ? 'container-fluid' : 'container'; ?>">

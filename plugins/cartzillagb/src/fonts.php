<?php
/**
 * Loads the Google Fonts used in CartzillaGB blocks in the frontend.
 */
...
if ( ! class_exists( 'CartzillaGB_Google_Fonts' ) ) {
    class CartzillaGB_Google_Fonts {
		...
		public function enqueue_frontend_block_fonts() {
			if ( ! apply_filters( 'cartzillagb_enqueue_fonts', true ) ) {
				return;
			}
		...
		public function is_cartzillagb_block( $block_name ) {
			return strpos( $block_name, 'czgb/' ) === 0;
		}
		...
		public function gather_google_fonts( $blocks ) {
			...
			foreach ( $blocks as $block ) {
				...
				if ( $this->is_cartzillagb_block( $block['blockName'] ) ) {
		...
		public function enqueue_google_fonts( $google_fonts ) {
			...
			wp_enqueue_style( 'cartzillagb-google-fonts', $fonts_url ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}

	}

	new CartzillaGB_Google_Fonts();
}

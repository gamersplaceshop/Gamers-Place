...
if ( ! class_exists( 'CartzillaGB_Design_Library' ) ) {
	/**
	 * Class CartzillaGB Design Library
	 */
	class CartzillaGB_Design_Library {
		...
		public function __construct() {
			...
			add_filter( 'cartzillagb_design_library_retreive_body', array( $this, 'replace_dev_mode_urls' ) );
		}
		...
		public function delete_cache() {
			...
			delete_transient( 'cartzillagb_get_design_library' );

			// Delete block designs.
			global $wpdb;
			$transients = $wpdb->get_col( "SELECT option_name FROM $wpdb->options WHERE option_name LIKE '_transient_cartzillagb_get_block_designs_%'" );
			...
			// Delete designs.
			$transients = $wpdb->get_col( "SELECT option_name FROM $wpdb->options WHERE option_name LIKE '_transient_cartzillagb_get_design_%'" );
		...
		public function _get_design_library() {
			$designs = get_transient( 'cartzillagb_get_design_library' );

			// Fetch designs.
			if ( empty( $designs ) ) {
				$response = wp_remote_get( $this->get_cdn_url() . 'library/library.json' );
				$content = wp_remote_retrieve_body( $response );
				$content = apply_filters( 'cartzillagb_design_library_retreive_body', $content );
				$designs = json_decode( $content, true );

				// Cache results.
				set_transient( 'cartzillagb_get_design_library', $designs, DAY_IN_SECONDS );
			}

			return apply_filters( 'cartzillagb_design_library', $designs );
		}
		...
		public function get_block_designs( $request ) {
			...
			$designs = get_transient( 'cartzillagb_get_block_designs_' . $block );
			...
			if ( empty( $designs ) ) {
				...
				// Cache results.
				set_transient( 'cartzillagb_get_block_designs_' . $block, $designs, DAY_IN_SECONDS );
			}

			$designs = apply_filters( 'cartzillagb_block_design_' . $block, $designs );
		...
		public function get_design( $request ) {
			...
			$design = get_transient( 'cartzillagb_get_design_' . $design_id );
			...
			if ( empty( $design ) ) {
				...
				// Cache results.
				set_transient( 'cartzillagb_get_design_' . $design_id, $design, DAY_IN_SECONDS );
			}

			$design = apply_filters( 'cartzillagb_design_' . $design_id, $design );
		...
		public function replace_dev_mode_urls( $content ) {
			if ( $this->is_dev_mode() ) {
				$content = str_replace(
					trailingslashit( CZGB_CLOUDFRONT_URL ),
					trailingslashit( plugin_dir_url( CZGB_DLH_LIBRARY_FILE ) ),
					$content
				);
			}
			return $content;
		...
		public function get_cdn_url() {
			if ( $this->is_dev_mode() ) {
				return trailingslashit( plugin_dir_url( CZGB_DLH_LIBRARY_FILE ) );
			} else {
				return trailingslashit( CZGB_CLOUDFRONT_URL );
			}
		}

		public function is_dev_mode() {
			$dev_env = defined( 'WP_ENV' ) ? WP_ENV === 'development' : false;
			$export_tool_installed = defined( 'CZGB_DLH_LIBRARY_FILE' );
			$library_dev_mode_toggled = !! get_option( 'cartzillagb_library_dev_mode' );
		...
		public function set_dev_mode( $request ) {
			...
			update_option( 'cartzillagb_library_dev_mode', $dev_mode );

			return rest_ensure_response( true );
		}
	}

	new CartzillaGB_Design_Library();
}

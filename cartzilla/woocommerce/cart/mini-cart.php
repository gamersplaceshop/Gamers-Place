...
<?php if ( ! WC()->cart->is_empty() ) : ?>
	...
	>
		...
		foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) :
			...
			if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_widget_cart_item_visible', true, $cart_item, $cart_item_key ) ) :
				...
				?>
				<div class="woocommerce-mini-cart-item <?php echo esc_attr( apply_filters( 'woocommerce_mini_cart_item_class', 'widget-cart-item border-bottom py-2', $cart_item, $cart_item_key ) ); ?>">
					<?php
					echo apply_filters( // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
						'woocommerce_cart_item_remove_link',
						sprintf(
							'<a href="%s" class="remove remove_from_cart_button close text-danger" aria-label="%s" data-product_id="%s" data-cart_item_key="%s" data-product_sku="%s"><span aria-hidden="true">&times;</span></a>',
					...
					<div class="media align-items-center">
						<?php if ( empty( $product_permalink ) ) : ?>
							<div class="mr-2" style="max-width: 64px;">
								<?php echo apply_filters( 'woocommerce_cart_item_thumbnail', $_product->get_image(), $cart_item, $cart_item_key ); ?>
							</div>
						<?php else : ?>
							<a class="d-block mr-2" href="<?php echo esc_url( $product_permalink ); ?>" style="max-width: 64px;">
								<?php echo apply_filters( 'woocommerce_cart_item_thumbnail', $_product->get_image(), $cart_item, $cart_item_key ); ?>
							</a>
						<?php endif; ?>
						<div class="media-body">
							...
							<div class="widget-product-meta">
								...
								<span class="text-muted"><?php echo apply_filters( 'woocommerce_widget_cart_item_quantity', sprintf( '&times; %s', $cart_item['quantity'] ), $cart_item, $cart_item_key ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span>
							</div>
						</div>
					</div>
				</div>
	...
	<div class="woocommerce-mini-cart__total total d-flex flex-wrap justify-content-between align-items-center py-3">
		<div class="font-size-sm mr-2 py-2">
			<span class="text-muted"><?php esc_html_e( 'Subtotal', 'cartzilla' ); ?>:</span>
			<span class="text-accent font-size-base ml-1"><?php echo WC()->cart->get_cart_subtotal(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span>
		</div>
		<a class="btn btn-outline-secondary btn-sm" href="<?php echo esc_url( wc_get_cart_url() ); ?>">
			<?php echo esc_html_x( 'Expand cart', 'front-end', 'cartzilla' ); ?>
	...
	<div class="woocommerce-mini-cart__buttons buttons">
		<a class="btn btn-primary btn-sm btn-block" href="<?php echo esc_url( wc_get_checkout_url() ); ?>">
			<i class="czi-card mr-2 font-size-base align-middle"></i>
			<?php esc_html_e( 'Checkout', 'cartzilla' ); ?>
		</a>
...
<?php else : ?>
	...
	<div class="woocommerce-mini-cart__empty-message pt-2 text-center">
		...
		<p class="text-muted font-size-sm mb-0"><?php esc_html_e( 'No products in the cart.', 'cartzilla' ); ?></p>
	</div>
